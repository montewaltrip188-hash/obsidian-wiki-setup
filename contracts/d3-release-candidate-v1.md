...
`.github/workflows/d3-macos-candidate.yml` 使用标准托管 runner `macos-15-intel` 验收 `macos-x64`，使用 `macos-15` 验收 `macos-arm64`。控制 Job 从三个精确提交重建两次可复现候选；所有第三方 Action 锁定完整 commit，不使用仓库 secret。两份完成回执分别通过 GitHub artifact attestation 建立 workflow、repository、commit、runner 与产物之间的可验证来源关系。正式接受前必须运行 `gh attestation verify`，并用 `--signer-workflow jiegeng333/obsidian-wiki-setup/.github/workflows/d3-macos-candidate.yml` 限定签发工作流。
...
`release/d3_release.py prepare` 必须先用 `gh attestation verify --bundle` 验证两份 macOS 回执及其离线 Sigstore bundle，并锁定仓库、signer workflow、source commit、signer commit 和非自托管 runner。通过后组装两个候选 ZIP、两份 SBOM、D2 计划、三份回执和两份 attestation bundle，共 10 个逐文件记录。规范化 `release-manifest.json` 要求 `RSA-SHA256-PKCS1-v1_5` 分离签名，并在签名前写入已独立确认的生产公钥指纹 `key_id`。维护者在 Windows 签名机使用 `release/sign-manifest.ps1`；RSA 不得低于 3072 位，生产签名私钥不得进入仓库、候选、CI、日志或回执，仓库只允许保存作为外部信任根的公钥。Windows 使用 `release/verify-manifest.ps1`，macOS 使用 `release/verify-manifest.sh`，`release/d3_release.py verify` 还会验证公钥指纹、完整文件集合、长度、SHA-256，并重新交叉绑定 D2 计划、三平台回执、架构、候选和证明 bundle 摘要。
