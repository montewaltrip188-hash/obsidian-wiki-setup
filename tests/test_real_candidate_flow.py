...
class RealCandidateFlowTest(unittest.TestCase):
    def test_real_commits_build_deploy_install_verify_and_uninstall(self):
        ...
        with tempfile.TemporaryDirectory(prefix="d0-real-") as temporary:
            root = Path(temporary)
            plan = root / "plan.json"
            run(
                sys.executable,
            ...
            self.assertEqual(json.loads(deploy_receipt.read_text(encoding="utf-8"))["status"], "completed")
            ...
            plan_result = json.loads(
                run(sys.executable, manager, "plan", "--source", source, "--home", home).stdout
            )
            self.assertEqual(plan_result["action"], "install")
            install_result = json.loads(
                run(sys.executable, manager, "install", "--source", source, "--home", home).stdout
            )
            self.assertFalse(install_result["keyword_runtime_ready"])
            self.assertEqual(install_result["keyword_runtime_error"], "KEYWORD_RUNTIME_UNPROVISIONED")
            ...
            self.assertFalse((home / ".agents" / "packages" / "claudecode-wiki-skills").exists())
