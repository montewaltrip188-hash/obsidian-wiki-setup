...
当前稳定组合为产品 Schema `1.0.0`、Wiki Skill `2.1.0` 与 bundle `2.1.0`。Builder 会把三仓精确来源合成为客户候选根目录的 `bundle-manifest.json`，避免让仓库内文件自引用尚未产生的 commit 或 candidate ID。仓库中的 `stable` 只表示稳定来源合同已经固化；客户仍只接受经过 D2、D3、生产签名、不可变 tag 与 GitHub Release 闭环，并由 `release/stable.json` 明确指向的正式资产。U1 的只读边界见 `contracts/read-only-vault-update-v1.md`。
...
bundle `2.1.0` 已固化为 stable 来源合同，但这不等于客户已经可下载。正式交付顺序固定为：从三仓 stable 精确提交重跑 D2 → macOS 双架构真实 CI 与 GitHub 证明 → Windows 回执 → 生产 RSA 签名 → 不可变 tag 与 GitHub Release → 最后提交 `release/stable.json`，原子切换客户下载入口。
